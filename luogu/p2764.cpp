...
void solve() {
    int s = 0, t = 2*n+1, cc = n, u = s; memset(cnt, c = 0, sizeof(cnt));
    for (int i=0, x, y; i<m; ++i) cin >> x >> y, add_edge(x, y+n, 1);
    ...
    for (int i=1; i<=n; ++i) for (int j=0; j<cnt[i]; ++j) {
        const edge& ee = e[g[i][j]]; 
        if (ee.v != s && ee.flow) p[ee.v-n] = i, q[i] = ee.v-n;
    }
    for (int i=1; i<=n; ++i) if (!p[i]) {
        cout << i;
        for (int j=q[i]; j; j=q[j]) cout << ' ' << j;
        cout << endl;
    }
    cout << cc << endl;
}
