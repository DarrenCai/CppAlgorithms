...
#define N 1369
struct edge {int u, v, cap, flow;} e[166*N];
...
void solve() {
    ...
    for (int i=0; i<n; ++i) {
        ...
        while (c--) {
            int k, l, r; cin >> k >> l >> r; f += l; cs[m+i] += l; ct[k] += l; add_edge(k, m+i, r-l);
        }
    }
    add_edge(t, s, INF); s = t+1; t = s+1; n = t+1;
    for (int i=s-2; i>=0; --i) {
        if (cs[i]) add_edge(s, i, cs[i]);
        if (ct[i]) add_edge(i, t, ct[i]);
    }
    cout << (max_flow(s, t) == f ? max_flow(s-2, t-2) : -1) << endl << endl;
}
