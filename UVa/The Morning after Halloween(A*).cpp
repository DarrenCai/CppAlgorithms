...
void AStar() {
    while (!open.empty()) {
        ...
        if (n==1)
            for (char i=1; i<m[a[0][0]][a[0][1]].size(); ++i) {
                c[0][0] = a[0][0] + d[m[a[0][0]][a[0][1]][i]][0]; 
                c[0][1] = a[0][1] + d[m[a[0][0]][a[0][1]][i]][1];
                if (!expand()) return;
            }
        else if (n==2)
            for (char i=0; i<m[a[0][0]][a[0][1]].size(); ++i) {
                c[0][0] = a[0][0] + d[m[a[0][0]][a[0][1]][i]][0]; 
                c[0][1] = a[0][1] + d[m[a[0][0]][a[0][1]][i]][1];
                for (char j=0; j<m[a[1][0]][a[1][1]].size(); ++j) {
        ...
        else for (char i=0; i<m[a[0][0]][a[0][1]].size(); ++i) {
                c[0][0] = a[0][0] + d[m[a[0][0]][a[0][1]][i]][0]; 
                c[0][1] = a[0][1] + d[m[a[0][0]][a[0][1]][i]][1];
                for (char j=0; j<m[a[1][0]][a[1][1]].size(); ++j) {
                    c[1][0] = a[1][0] + d[m[a[1][0]][a[1][1]][j]][0];
                    c[1][1] = a[1][1] + d[m[a[1][0]][a[1][1]][j]][1];
                    for (char k=0; k<m[a[2][0]][a[2][1]].size(); ++k) {
