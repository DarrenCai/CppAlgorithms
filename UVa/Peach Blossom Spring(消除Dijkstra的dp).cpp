...
#define N 50
#define INF 1100000
int f[N][1<<(K<<1)], d[1<<K][1<<K], w[N][N], c[1<<K]={0}, n, m, k;
...
void solve() {
    ...
    for (int k=0; k<n; ++k) for (int i=0; i<n; ++i) for (int j=0; j<n; ++j) w[i][j] = min(w[i][j], w[i][k]+w[k][j]);
    for (int s=1; s<=x; ++s) {
        for (int i=0; i<n; ++i) for (int t=(s-1)&s; t>0; t=(t-1)&s) f[i][s] = min(f[i][s], f[i][t]+f[i][s^t]);
        for (int i=0; i<n; ++i) for (int j=0; j<n; ++j) f[i][s] = min(f[i][s], f[j][s]+w[i][j]);
