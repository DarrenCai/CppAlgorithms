...
#define N 101
#define L 102000
char s[L], p[N*10]; int sa[L], rk1[L], rk2[L], c[L], height[L], idx[L], q[L], f[N], len, n, t, kase = 0;
...
void print() {
    ...
    for (int i=1, x=1, y; s[sa[i]]<=26; ++i) {
        if (height[i] < len) {
            ...
            if (c==0 && (y = idx[sa[i-1]]) < n) f[y] = i, ++c;
            if ((y = idx[sa[i]]) < n && f[y] < x) ++c;
            f[y] = i;
        }
    }
}
...
void solve() {
    ...
    for (int i=1, c=0, x=1, y, h=n>>1, head=0, tail=len=0; s[sa[i]]<26; ++i) {
        if (height[i] <= len) {
