...
int main()
{
    ...
    while(cin>>maze && maze!="END"){
        ...
        node start=node(r0, c0), end=node(r1, c1); cross sc = cross(r0, c0, convert(c)); short d = F << 3*sc.d;
        visit.insert(sc); q.push_back(sc); nodes[start]=d; path.push_back(vector<node>(1, start));
        while(cin>>r1 && r1) {
        ...
        short head=0,tail=1;
        while(head < tail) {
            node t = node(q[head].r, q[head].c); if(!nodes.count(t)) continue;
        ...
        cout << maze << endl;
        const vector<node>& ans = path[path.size()-1];
        if(*(ans.rbegin()) == end) cout << "  No Solution Possible";
        else for(int i=0, len=ans.size(); i<len; ++i) {
            if(i%10==0) cout << ' ';
