...
bool topology(const char u, const char (&g)[52][52], char (&visit)[52]){
    visit[u] = -1;
    for(char v=0; v<52; ++v) if(g[u][v]){ if(visit[v] == -1) return true; if(!visit[v] && topology(v, g, visit)) return true;};
...
int main()
{
    ...
    while(cin >> n){
        ...
        for(unsigned short i=0; i<n; ++i){ char e[4];
            ...
            for(char j=0; j<4; ++j) for(char k=0; k<4; ++k) if(j!=k && e[j]>=0 && e[k]>=0) g[e[j]^1][e[k]] = 1;
        }
