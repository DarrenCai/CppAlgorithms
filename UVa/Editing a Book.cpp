...
short n, ops, kase=0; char p[9];

char idx(const char *s, char a) {
    for (char i=0; i<n; ++i) if (s[i] == a) return i;
    return 0;
}
...
bool IDAStar(const char* s, char d=0) {
    bool isAns = true;
    for (char i=0; isAns && i<n; ++i) if (s[i] != '1'+i) isAns = false;
            ...
            if ((!i || s[i] != s[i-1] + 1) && s[i+l] != s[i+l-1] + 1) {
                ...
                if (a<i+l && b<i+l) {
                    for (char j=i+l; j<n; ++j)
                        if ((j==n-1 || s[j+1] != s[j] + 1) && expand(s, d+1, i, l, j)) return true;
                } else {
...
bool expand(const char *s, char d, char i, char l, char j) {
    ...
    for (char k=0; k<n-1; ++k) if (s1[k]+1 != s1[k+1]) ++t;
    if (t <= 3*(ops-d) && IDAStar(s1, d)) return true;
    return false;
}
...
int main() {
    ...
    while (cin>>n && n) {
        for (short i=ops=0; i<n; ++i) cin >> p[i];
        while (!IDAStar(p)) ++ ops;
        cout << "Case " << ++kase << ": " << short(ops) << endl;
    }
    return 0;
}