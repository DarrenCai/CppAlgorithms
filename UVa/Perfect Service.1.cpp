...
void dp(short i = 1) {
    ...
    int s = 0, sj; d00[i] = n;
    for (short j=g[i].size()-1; j>=0; --j) if (d00[g[i][j]] >= n) ++s, sj=j;
    if (s > 1) return;
    if (s == 1) {
        d00[i] = d1[g[i][sj]];
        for (short j=g[i].size()-1; j>=0; --j) if (j != sj) d00[i] += d00[g[i][j]];
    } else for (short j=g[i].size()-1; j>=0; --j) d00[i] = min(d00[i], d01[i]-d00[g[i][j]]+d1[g[i][j]]);
}
