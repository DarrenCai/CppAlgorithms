...
#define N 26
#define T 37
struct edge {int u, v, cap, flow;} e[N*T];
int g[T][N], q[T], p[T], d[T], cur[T], num[T], cnt[T], cc[N], c, m, n, t; bool vis[T];
...
int solve() {
    cin >> n >> m;
    t = m+n; memset(cnt, c = 0, sizeof(cnt));
    for (int i=0; i<n; ++i) {
        int k, x; cin >> k; memset(cc, 0, sizeof(cc));
        while (k--) cin >> x, ++cc[x];
        for (int j=1; j<=m; ++j) {
            if (i) {
                if (cc[j] > 1) add_edge(i, j+n-1, cc[j]-1);
                else if (!cc[j]) add_edge(j+n-1, i, 1);
            } else {
                if (cc[j]) add_edge(0, j+n-1, cc[j]);
                add_edge(j+n-1, t, 1);
            }
        }
    }
